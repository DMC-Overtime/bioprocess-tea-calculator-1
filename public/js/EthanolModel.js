...
Input = [];
Input[7] = 0.51;// selling Price
Input[18] = 0.072; // GlucoseCosts[i];
const myChemical = new chemical();
myChemical.carbon = 2;
myChemical.hydrogen = 6;
myChemical.oxygen = 1;
...
Input[23] = 4; //Titer,
Input[24] = 200; //aveVolumtericRate,


            bioprocessOutputs = bioprocessopexcapex(Input);
            actPlantCapacity = bioprocessOutputs.plantCapacity;
            console.log(bioprocessOutputs.plantCapacity);
            bioprocessOutputs.totaCapitalInvestment = 0.50*actPlantCapacity;
            DCFOutput = BOO_DCF(Input,bioprocessOutputs);
            IRR = ((DCFOutput.IRR)*100).toFixed(2);
            console.log(IRR);
            console.log(bioprocessOutputs.opexperkg);
...
//          30-dspPercentofOpex,          31-dspPercentofCapex ];
  ...
  // The following are Calculations based on the Fermentation Targets
   ...
   // Cumulative O2/Air Need Needed
    cumulativeProductO2 = 0;// (((finalTiter*totalAnnualFermWorkingVolume)/productYieldCoefficientO2)/32)*1000; //mmoles O2 needed
    cumulativeBiomassO2 = 0;// (((finalBiomassConcentration*totalAnnualFermWorkingVolume)/biomassYieldCoefficientO2)/32/BiomassYieldFraction)*1000;// mmoles O2 needed for Biomass
    cumulativeByProductO2 = 0;// (((cumulativeByProductGlucose)/FeedstockMW)*1000)*6*1000;//mmoles O2 needed for biomass and product inefficiencies
    cumulativeO2 = 0;//cumulativeBiomassO2 + cumulativeProductO2 + cumulativeByProductO2;// mmoles O2 needed
    averageOTR = 0;// (cumulativeO2/annualFermentationUpTime/totalAnnualFermWorkingVolume);// average mmoles/L-hr O2 needed
    cumulativeAir = 0;// cumulativeO2/(9.375*1000) ;// m3 of air needed, 9.375 mmoles O2 per liter of air
    averageAnnualAirflow = 0;// (
    AnnualCostOfAirperM3 = 0; //
    annualCostOfCompressedAir = 0;//

    // max OTR & Kla

    // Biomass OTR
    maxBiomassOTR = 0;// (((1/biomassYieldCoefficientO2)/BiomassYieldFraction)*(1000/32)*averageLogisticGrowthRate/4)*finalBiomassConcentration;  // (mmoles O2/ L-hr), assumes logistic growth per tank
    maxByProductOTR = 0;// (((1/byproductYieldCoefficientO2)/(1-BiomassYieldFraction))*(1000/32)*averageLogisticGrowthRate/4)*finalBiomassConcentration;  // (mmoles O2/ L-hr), assumes logistic growth per tank
    ...
    maxOTR = 0;//  maxProductOTR +maxByProductOTR+ maxBiomassOTR; // mmoles/L-hr per tank
    maxOxygenFlowRate = 0;// maxOTR*maxVesselWorkingVolume; // mmoles/hr per tank
    maxAirFlowRate = 0;// maxOxygenFlowRate/9.375/0.75/1000/3600; // (standard m3 air per second) per tank, 9.375 mmoles O2 per liter of air, divided by fraction which is taken up by liquid
    maxKla = 0;// (maxOTR/0.15)/3600; //(sec-1) assumes air as sole oxygen source, assumes logistic growth per tank and maximal driving force at 75% consumption (0.2 - 0.05mmoles O2/L)

    // Mass transfer requirements (Stirred Tank) assumes $2/kg of O2 delivered, 0.233 kg O2 per kg Air
    cumulativeAirkg = 0; //cumulativeAir*1.225; //
    annualMassTransferPowerNeed = 0; //1.8*0.233*cumulativeAirkg; //  KWh , assumes  O2 delivered, 0.233 kg O2 per kg Air % Watts,  Humbird et al.
    annualCost0fMassTransfer = 0;//annualMassTransferPowerNeed*ElectricityCost; //
   ...
   // Cooling Tower Water
    ...
    annualCostOfSterilization = 0;// (mediaSterilizationEnergyConsumption/1055056)*NaturalGasCost; // $/year
    ...
    annualCIPCosts = 0;// annualNaOHCosts+annualPeraceticCosts;
    ...
    annualCostOfHeatKill =  0;// (heatKillEnergyConsumption/1055056)*NaturalGasCost; //  $/year
...
// Indirect Costs
   ...
   // Fermentation Opex Cost Summary
    annualUtilityCosts = annualCostOfHeatKill  + annualCostOfCentrifugation; //
    annualRawMaterialCosts = annualCostOfMedia  + annualCostOfNH3 + annualCostOfGlucose + annualWaterCosts;  // + annualCIPCosts; //
    annualLaborCosts = 0.016*plantCapacity; //
    annualAdditionalFixedCosts = 0; // (per NREL2013 x inflation factor); //
    ...
    console.log(annualUtilityCosts);
